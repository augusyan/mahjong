...
# @update: + tensorboard to draw the loss-acc
...
from Resnet_simple_v1 import Resnet_pure
from Resnet_simple_v1 import fc
...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self, loss_type):
        ...
        plt.savefig('loss/20180528/sys_king/sys_king_res34_e50d5.png')
        # plt.show()
...
units = 1024  # fc神经元数
num_epoch = 50  # 训练轮数
model_save_path = 'model/20180528/sys_king_res34_e50d5.h5'  # 模型保存地址
model_pic = 'model/20180528/sys_king_res34_e50d5.png'  # 绘图保存地址
res_block = 7  # 残差块
log = './log/20180528/op_non_king/op_king_dense_e50d5.log'  # log 地址
# model_restore = 'model/20180528/sys_king_resnet_simple_e0-10d5.h5'
# data path
x_train = np.loadtxt('mj_data/data_201804/king_20180528/output_X_train_pic.txt', delimiter=' ', dtype=np.float16)
x_test = np.loadtxt('mj_data/data_201804/king_20180528/output_X_test_pic.txt', delimiter=' ', dtype=np.int32)
y_train = np.loadtxt('mj_data/data_201804/king_20180528/output_Y_train.txt', delimiter=' ', dtype=np.float16)
y_test = np.loadtxt('mj_data/data_201804/king_20180528/output_Y_test.txt', delimiter=' ', dtype=np.int32)

y_train = np_utils.to_categorical(y_train, num_classes=output_features)
y_test = np_utils.to_categorical(y_test, num_classes=output_features)
...
plot_model(model, to_file='Res34_v1.png', show_shapes=True)
adam = Adam(lr=1e-4)
...
hist_log = model.fit(x_train, y_train, batch_size=batch, epochs=num_epoch, validation_data=(x_test, y_test),
                     callbacks=[TensorBoard(log_dir='log/20180528/sys_king/sys_king_res34_e50d5')])
# hist_log = model.fit(x_train, y_train, batch_size=batch, epochs=num_epoch, validation_data=(x_test, y_test),
#                      callbacks=[history])
print('Saving Log -------------')
