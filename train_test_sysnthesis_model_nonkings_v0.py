# -*- coding: utf-8 -*-
# @Time    : 17-12-27 上午9:26
# @Author  : Yan
# @Site    : 
# @File    : train_test_sysnthesis_model_nonkings_v0.py
# @Software: PyCharm Community Edition
# @Function:   hidden layer units 1200
...
# data path
trainData_tmp = np.loadtxt('data/synthesis/sys_nonkings_train_9.txt', delimiter=' ', dtype=np.float16)
# trainData_tmp = np.loadtxt('data/data223_train.txt', delimiter=' ', dtype=np.float16)
trainData = trainData_tmp
testData_tmp = np.loadtxt('data/synthesis/sys_nonkings_test_9.txt', delimiter=' ', dtype=np.float16)
...
model_save_name = '20171227_non_king_v0_layer2_d10'
model_restore_path = "save/20171227_non_king_v0_layer2_d10_c8_100.ckpt"
...
layer1 = add_layer(x, input_features, 1200, 1, 'relu')
layer2 = add_layer(layer1, 1200, 1200, 2, 'relu')
# layer3 = add_layer(layer2, 1000, 1000, 3, 'relu')
output = add_layer(layer2, 1200, output_features, 3)
    ...
    if continue_trian:
        with tf.Session() as sess:
            ...
            for epoch in range(num_epochs):
                ...
                writer.add_summary(result, epoch)  # result是summary类型的，需要放入writer中，i步数（x轴）
                if epoch % 20 == 0:
                    # saver_path = saver.save(sess, 'save/20171227_non_king_v0_layer2_d10_c2_%d.ckpt' % epoch)
                    saver_path = saver.save(sess, 'save/' + model_save_name + '_c9_%d.ckpt' % epoch)
